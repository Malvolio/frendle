...
import type { MatchCandidate, Session, TimeSlot } from "../shared/types.ts";
import {
  createErrorResponse,
...
const matchCurrentUser = async (): Promise<Response> => {
  ...
  try {
    ...
    for (const candidate of candidates) {
      ...
      if (candidateSessions?.length >= MAX_SESSIONS) continue;

      const candidateAvailability =
        candidate.availability?.map((a) => a.hour_of_week) || [];
      ...
      const overlappingSlots = findOverlappingSlots(
        ...
        candidate.private_profiles.timezone
      );
      ...
      if (overlappingSlots.length > 0) {
        validMatches.push({
          candidate: {
            id: candidate.id,
            name: candidate.name,
            timezone: candidate.private_profiles.timezone,
            offset_hours: candidate.timezones.offset_hours,
...
Session Details:
Date: ${formatDateForEmail(new Date(session.scheduled_for), userPrivateProfile.timezone)}
...
Session Details:  
Date: ${formatDateForEmail(new Date(session.scheduled_for), match.candidate.timezone)}
