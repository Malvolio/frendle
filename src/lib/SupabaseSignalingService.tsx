...
import { Signaling } from "./useWebRTC"; // Import from your WebRTC library
...
export class SupabaseSignalingService implements Signaling {
  ...
  // Event handlers
  private onOfferHandler: ((offer: RTCSessionDescriptionInit) => void) | null =
    null;
  private onAnswerHandler:
    | ((answer: RTCSessionDescriptionInit) => void)
    | null = null;
  ...
  private async setupRealtimeSubscription(): Promise<void> {
    const { data } = await supabase
      ...
      .eq("id", this.sessionId)
      .single();
    this.lastSessionUpdate = data as SessionRow;
    ...
    this.channel = supabase
      ...
      .on(
        ...
        (payload) => {
          this.lastSessionUpdate = payload.new as SessionRow;
          this.handleSessionUpdate(payload.new as SessionRow);
        }
      )
      .subscribe();
  }
   ...
   */
  private handleSessionUpdate(session: SessionRow): void {
    // Handle offer (for guests)
    if (!this.isHost && session.offer && this.onOfferHandler) {
      this.onOfferHandler(session.offer);
    }

    // Handle answer (for hosts)
    if (this.isHost && session.answer && this.onAnswerHandler) {
...
/**
 * Factory function to create and initialize a Supabase signaling service
 */
export const createSupabaseSignaling = async (
  sessionId: string,
  isHost: boolean,
  userId: string
): Promise<SupabaseSignalingService> => {
...
export const useSupabaseSignaling =
  (sessionId: string, isHost: boolean, userId: string) => () =>
    createSupabaseSignaling(sessionId, isHost, userId);
